...
const Workspace = ({ children }) => {
  ...
  const {
    data: userData, error: userDataError, mutate: userDataMute,
  } = useSWR('/api/users', fetcher, { dedupingInterval: 2000 });
  const {
    data: channelData, error: channelDataError, mutate: channelDataMutate,
    ...
    <Header>
      <RightMenu>
          <span onClick={onClickUserProfile}>
            ...
            {showUserMenu && (<Menu style={{ right: 0, top: 38 }} onCloseModal={onClickUserProfile}>
              <ProfileModal>
    ...
    <WorkspaceWrapper>
      <Workspaces>
        {userData?.Workspaces.map((workspace) => {
          return (<NavLink key={workspace.id} to={`/workspace/${123}/channel/일반`}>
